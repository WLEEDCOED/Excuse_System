...
// Ø§Ù„ØªØ­Ù‚Ù‚ Ù…Ù† Ø£Ù† Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù… Ù‡Ùˆ Ø·Ø§Ù„Ø¨
        $sql_check_student = "SELECT * FROM students WHERE id = ?";
        ...
        if ($result_check_student->num_rows == 0) {
            echo "ØµÙ„Ø§Ø­ÙŠØ§Øª ØºÙŠØ± ÙƒØ§ÙÙŠØ©.";
            exit;
        }

// Ø¬Ù„Ø¨ Ø§Ù„Ù…ÙˆØ§Ø¯ Ù…Ù† Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª
        $sql_classes = "SELECT name FROM Course";
...
// Ø¹Ù†Ø¯ Ø¥Ø±Ø³Ø§Ù„ Ø§Ù„Ù†Ù…ÙˆØ°Ø¬
    ...
    if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($_FILES["file"]["tmp_name"], $target_file)) {
            ...
            // Ø¬Ù„Ø¨ Ø§Ù„Ù…Ø³Ø¤ÙˆÙ„ Ø§Ù„Ø£ÙˆÙ„ Ø§Ù„Ø°ÙŠ ÙŠÙ†ØªÙ…ÙŠ Ø¥Ù„Ù‰ Ù†ÙØ³ Ø§Ù„Ù‚Ø³Ù… ÙˆÙ„Ø¯ÙŠÙ‡ role_id = 1 (admin)
            ...
            if ($result_admin->num_rows > 0) {
                ...
                $response['error'] = "Ù„Ø§ ÙŠÙˆØ¬Ø¯ Ù…Ø³Ø¤ÙˆÙ„ ÙÙŠ Ù†ÙØ³ Ø§Ù„Ù‚Ø³Ù….";
                echo json_encode($response);
                exit;
            }

            // Ø¥Ø¯Ø®Ø§Ù„ Ø¨ÙŠØ§Ù†Ø§Øª Ø§Ù„Ø·Ù„Ø¨ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§ØªØŒ Ø¨Ù…Ø§ ÙÙŠ Ø°Ù„Ùƒ created_at
            $sql = "INSERT INTO final_exam (student_id, class, date, excuse, description, file_path, status, current_admin, current_admin_table, created_at) 
                    VALUES (?, ?, ?, ?, ?, ?, 'Pending', ?, ?, CURRENT_TIMESTAMP)";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("isssssis", $student_id, $class_name, $exam_date, $excuse, $description, $stored_file_path, $current_admin_id, $current_admin_table);

            if ($stmt->execute()) {
                $response['success'] = "ØªÙ… Ø¥Ø±Ø³Ø§Ù„ Ø·Ù„Ø¨Ùƒ Ø¨Ù†Ø¬Ø§Ø­.";
            } else {
...
<head>
    <meta charset="UTF-8">
    <title>Ø¥Ø±Ø³Ø§Ù„ Ø§Ù„Ø§Ù…ØªØ­Ø§Ù† Ø§Ù„Ù†Ù‡Ø§Ø¦ÙŠ - Ø¨ÙˆØ§Ø¨Ø© Ø§Ù„Ø·Ù„Ø§Ø¨</title>
...
<body>
    ...
    <div class="content-wrapper">
        <div class="form-container">
            ...
            <h2>Ø¥Ø±Ø³Ø§Ù„  Ø§Ù„Ø¹Ø°Ø± Ù„Ù„Ø§Ø®ØªØ¨Ø§Ø± Ø§Ù„Ù†Ù‡Ø§Ø¦ÙŠ</h2>
            ...
            <form id="requestForm" method="post" enctype="multipart/form-data">
                ...
                <input type="date" name="date" placeholder="ØªØ§Ø±ÙŠØ® Ø§Ù„Ø§Ù…ØªØ­Ø§Ù†" required>
                <input type="text" name="excuse" placeholder="Ø§Ù„Ø¹Ø°Ø±" required>
                <textarea name="description" placeholder="Ø§Ù„ÙˆØµÙ" rows="4" required></textarea>
